...
public interface CarRepository extends JpaRepository<Car, Long> {
    ...
    List<Car> findByAirPortAmongNotBookedCars(@Param("airport") String airport, @Param("carIds") List<Long> carIds);

    @Query("select c from Car c where c.airport.city.name = :city " +
            "and (c.bookingState = false or c.id not in :carIds )")
    List<Car> findByCityAmongNotBookedCars(@Param("city") String city, @Param("carIds") List<Long> carIds);
}
